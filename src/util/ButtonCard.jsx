...
class ButtonCard extends React.Component {
    ...
    handleDownload = async () => {
        ...
        const { photo, img } = this.props;
        // Gunakan 'photo' atau 'img' mana saja yang tersedia
        const data = photo || img;
        const { size } = this.state;

        // Validasi awal: Jika data tidak ada, jangan lanjutkan
        if (!data) {
            alert("Data gambar tidak ditemukan!");
            return;
        }
        ...
        try {
            ...
            alert(`Gagal: ${err.message}. Pastikan Backend aktif di Port 5000.`);
        } finally {
    ...
    render() {
        return (
            /* Gabungkan Select dan Button agar satu baris rapi */
            <div className="flex items-center bg-white border border-gray-300 rounded-md shadow-sm overflow-hidden">
                <select
                    value={this.state.size}
                    onChange={(e) => this.setState({ size: e.target.value })}
                    className="text-[10px] md:text-xs font-bold uppercase px-2 py-2 bg-transparent border-r border-gray-200 outline-none cursor-pointer hover:bg-gray-50 transition-colors"
                >
                ...
                <button
                    ...
                    className="p-2 md:p-2.5 bg-white hover:bg-gray-50 active:bg-gray-100 transition-all flex items-center justify-center disabled:opacity-50"
                >
                    {this.state.loading ? (
